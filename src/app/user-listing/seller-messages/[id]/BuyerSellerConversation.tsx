...
const BuyerSellerConversation: React.FC<MsgIdProps> = ({
  idd,
  ...
  useEffect(() => {
    intervalRef.current = setInterval(hitMessageApi, 90 * 1000);
    return () => {
  ...
  return (
    <div>
      <div>
        <div className="h-[10rem] xl:h-[22rem] md:h-[23rem] lg:h-[23.5rem] overflow-y-auto pe-3 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-300 scroll-smooth">
          {/* <div className="min-h-[19rem] xl:min-h-[26rem] md:min-h-[22rem] lg:min-h-[22rem] overflow-y-auto pe-3 scrollbar-thin scrollbar-thumb-gray-500 scrollbar-track-gray-300 scroll-smooth"> */}
          <div className="flex flex-col mb-2">
            {/* <p className={`mb-0 flex `}>
              {conversationMessages[0]?.senderUser?.fullname
                ? conversationMessages[0]?.senderUser?.fullname
                : "Seller"}
            </p> */}
          ...
          {conversationMessages.length === 0 ? (
            ...
            conversationMessages.slice(1).map((item: any, key: number) => {
              ...
              return (
                ...
                //       className={`flex ${
                //         item.isBuyer ? "justify-end" : "justify-start"
                //       } ${item.isBuyer ? "text-bright-green" : "text-white"}`}
                //     >
                //       {item.isBuyer ? "12:38" : "1:00"}
                //     </p>
                //   </div>
                // </div>
                <div key={key}>
