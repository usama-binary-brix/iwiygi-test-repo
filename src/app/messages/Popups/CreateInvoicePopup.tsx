...
const CreateInvoicePopup = ({
  ...
  const [uspsShippingRate, setUspsShippingRate] = useState<any>([]);
  ...
  const invoiceId = messages[0]?.invoice?.id
  ...
  const handlecreateInvoicePostRequest = async () => {
    ...
    try {
      ...
      const selectedOption = uspsShippingRate?.find(
        (option: any) => option.object_id === selectedShippingOption
      );
  
      const sanitizedData = {
        ...createInvoiceData,
        shipmentAmount: Number(selectedOption.amount),
      ...
      const payload = {
        ...sanitizedData,
        itemName: itemName,
        // invoiceeName: itemName,

        invoiceeEmail: messages[0]?.recieverUser?.email,
        invoiceeName: messages[0]?.recieverUser?.username,
        buyerId: messages[0]?.recieverUser?.id,
        listingId: messages[0]?.listing?.id,
        messageId: messages[0]?.id,
        labelData: selectedOption ? {
          invoiceId: invoiceId, // Will be set by backend
          objectId: selectedOption.object_id,
          objectOwner: selectedOption.object_owner,
          shipment: selectedOption.shipment,
          amount_local: selectedOption.amount_local,
          provider: selectedOption.provider,
          carrier_account: selectedOption.carrier_account,
          duration_terms: selectedOption.duration_terms || "",
          estimated_day: selectedOption.estimated_days?.toString() || "",
          service_level: selectedOption.servicelevel?.name,

          // // Additional fields from the object
          // object_created: selectedOption.object_created,
          // currency: selectedOption.currency,
          // currency_local: selectedOption.currency_local,
          // attributes: selectedOption.attributes,
          // zone: selectedOption.zone,
          // test: selectedOption.test,
          // servicelevel_token: selectedOption.servicelevel?.token,
          // included_insurance_price: selectedOption.included_insurance_price
        } : null
      };
      ...
      const data = await response.json();
      toast.success("Invoice Successfully Created and Sent to Buyer");
                                                  ...
                                                  <td className="py-3 pl-2">
                                                    <label className="flex items-center cursor-pointer">
                                                      <input
                                                        ...
                                                        checked={selectedShippingOption === option.object_id}
                                                        onChange={() => setSelectedShippingOption(option.object_id)}
